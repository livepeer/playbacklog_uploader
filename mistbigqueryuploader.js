...
if (argv.addr){process.env.ADDR = argv.addr;}
if (argv.dataset){process.env.DATASET = argv.dataset;}
if (argv.table){process.env.TABLE = argv.table;}
if (argv.gauth){process.env.GOOGLE_APPLICATION_CREDENTIALS = argv.gauth;}
...
s.on('message', (msg, rinfo) => {
  ...
  bq.dataset(process.env.DATASET).table(process.env.TABLE).insert([{log: JSON.stringify(json), timestamp: Date.now(), id: crypto.randomUUID()}]).catch((e) => {
    console.warn(`${e}`);
    if (e.errors){
      for (err in e.errors){
